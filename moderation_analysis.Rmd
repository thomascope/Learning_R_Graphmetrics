...
covariate_frame <- read.csv("../Data analysis/covariates.csv", header=TRUE)

for(level in unique(covariate_frame$Group)){
  covariate_frame[paste("dummy", level, sep = "_")] <- ifelse(covariate_frame$Group == level, 1, 0)
}

covariate_frame_justpats <- read.csv("../Data analysis/covariates_justpats.csv", header=TRUE)
...
# describeBy(covariate_frame, covariate_frame$Group)
# 
# example <- lm(degreeWt ~ dummy_2 + dummy_3, data=covariate_frame)
# summary(example)
